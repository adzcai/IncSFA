...
def testincsfa(animate=False):
    ...
    if PYQT:
        ...
        # init curves
        curves1 = [
            pg.PlotCurveItem(pen=(i, output_dim * 1.3)) for i in range(output_dim)
        ]
        ...
        if animate:
            view.show()
            trn = trainer.itrain(
                input_data, iterval=iterval, monitor_keys=["slowFeatures"]
            )
            ...
            def animloop():
                ...
                if not rtnval:
                    ...
                    print("")
                    print(
                        "Total Time for %d iterations: " % (iterval),
                        time.time() - _tcnt,
                    )
                    print(
                        "Success status: ",
                        np.allclose(
                            dcosines[cnt[0] - 1], np.ones(output_dim), atol=1e-2
                        ),
                    )
                    print(
                        "Maximum error: ",
                        np.max(np.abs(np.ones(output_dim) - dcosines[cnt[0] - 1])),
                    )
                    return
                v = trainer.monitorVar["slowFeatures"][0]
                out = node.execute(input_data)
                for dim in range(output_dim):
                    dcosines[cnt[0], dim] = np.abs(np.dot(v[dim], bv[dim].T)) / (
                        np.linalg.norm(v[dim]) * np.linalg.norm(bv[dim])
                    )
            ...
            print(
                "Success status: ",
                np.allclose(dcosines[-1], np.ones(output_dim), atol=1e-2),
            )
