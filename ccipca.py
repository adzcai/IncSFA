...
class CCIPCANode(object):
    ...
    **Instance variables of interest**
    ...
    def __init__(self, input_dim, output_dim, **kwargs):
        ...
        )  # Internal Eigen Vector (unNormalized)
        self._d = np.sum(np.absolute(self._v) ** 2, axis=-1) ** (
            1.0 / 2
        )  # Internal Eigen Values
        self._vn = self._v / self._d.reshape(
            self._v.shape[0], 1
        )  # Internal Eigen Vector (Normalized)
    ...
    def _amnesic(self, _i):
        [n1, n2, m, C] = [20.0, 200.0, 2000.0, 3.0]
        if _i < n1:
            l = 0
