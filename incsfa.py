...
class IncSFANode(object):
    ...
    **Instance variables of interest**
    ...
    def __init__(
        self, input_dim, whitening_output_dim, output_dim, eps=0.001, **kwargs
    ):
        ...
        self.xavg = signalAvgNode(
            mode=self.kwargs.get("avgMode", "Avg"), avgN=self.kwargs.get("avgN", 1000)
        )
    ...
    def update(self, x, newEpisode=False, **kwargs):
        ...
        if self.eps is None:
            ...
            self.mcanode.gamma = np.asscalar(
                1.2 * self.singlepcanode.d / np.mean(self.zbvar.update(zdot))
            )  # set gamma > lamda_1
            self._curreps = [
                0.2 / (self.mcanode.gamma[0] * ((1 / 1.2) + eig_id))
                for eig_id in range(self.output_dim)
            ]
        else:
            self.mcanode.gamma = 1.2 * (0.2 / self.eps)
            self.mcanode.eps = self._epsUpdate(self.n)
            self._curreps = [
                self.eps / (1 + eig_id * 1.2) for eig_id in range(self.output_dim)
            ]
    ...
    **Instance variables of interest**
    ...
    def __init__(
        ...
        try:
            ...
            self.expnode = mdp.nodes.QuadraticExpansionNode(input_dim)
        except ImportError:
            raise Exception("Install MDP toolkit to use IncSFA2Node")
        super(IncSFA2Node, self).__init__(
            self.expnode.output_dim, whitening_output_dim, output_dim, eps, **kwargs
        )

    def update(self, x, newEpisode=False):
        return super(IncSFA2Node, self).update(
            self.expnode(np.atleast_2d(x)), newEpisode
        )
