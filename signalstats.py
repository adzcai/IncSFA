...
class signalAvgNode(object):
    def __init__(self, mode="Avg", avgN=1000.0):
        self.avg = 0
        self.n = 0
        self.mode = mode
        self.N = avgN
    ...
    def update(self, x):
        ...
        if self.mode == "Avg":
            self.avg = (self.n * self.avg + input_data.sum(axis=0)) / (
...
class signalVarNode(object):
    ...
    def update(self, x):
        ...
        if (self.n + input_data.shape[0]) > 1:
            ...
            x_var = delta * x_mean
            self.var = ((self.n - 1) * self.var + x_var.sum(axis=0)) / (
                self.n - 1 + input_data.shape[0]
            )
        self.n += input_data.shape[0]
        return self.var
