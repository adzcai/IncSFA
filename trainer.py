...
def updateProgressBar(val, name, barLength):
    ...
    text = "\r[{0}] {1}% {2}".format(
        "#" * block + "-" * (barLength - block), int(val * 100), status
    )
    ...
    **Inputs**
    ...
    def __init__(self, ulnode, mode="Incremental", **kwargs):
        ...
        if not (self.mode in self.ulnode._validTrainingModes):
            raise Exception(
                "Invalid training mode! Valid training modes: %s"
                % (str(self.ulnode._validTrainingModes))
            )
        ...
        if self.mode is "Incremental":
            ...
            raise Exception(
                "Unrecognized mode! Training modes: Incremental, BlockIncremental, Batch "
            )
    ...
    def train(self, x, iterval=1, **kwargs):
        ...
        if x.shape[0] is 1:
            iterval = 1  # overide iterval value when input is just a single sample
